...
Once development on a new stable version is complete, the following steps will prepare the version for release. For purposes of illustration, we will assume we are preparing the stable release of version 7.7.1.
...
3.  Merge any outstanding PRs that have been reviewed and accepted for the milestone (e.g. `elemental-7.7.1`).
...
5.  Prepare the release, if you wish you can do a dry-run first by specifying `-DdryRun=true`:
    ...
    What is the release version for "Elemental"? (xyz.elemental:elemental) 7.7.1: :
    What is SCM release tag or label for "Elemental"? (xyz.elemental:elemental) elemental-7.7.1: :
    What is the new development version for "Elemental"? (xyz.elemental:elemental) 7.8.0-SNAPSHOT: :
    ```
...
7.  Update the stable branch (`gold`) of Elemental to reflect the latest release:
    ```bash
    $ git push origin elemental-7.7.1:gold
    ```
...
7. Send an email to the `Elemental` mailing list announcing the release with a title similar to `[ANN] Release of Elemental 7.7.1`, copy and paste the release notes from the blog into the email and reformat appropriately (see past emails).
