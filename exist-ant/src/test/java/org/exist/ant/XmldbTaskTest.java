...
public class XmldbTaskTest extends AbstractTaskTest {
    ...
    public void fileSetup() throws XMLDBException {
        try (final Collection col = existEmbeddedServer.createCollection(existEmbeddedServer.getRoot(), TEST_COLLECTION_NAME)) {
            ...
            try (final Resource res = col.createResource(TEST_RESOURCE_NAME, XMLResource.class)) {
                res.setContent("<test>hello <subject>world</subject></test>");
                col.storeResource(res);
            }

            try (final Resource binResource = col.createResource(BIN_TEST_RESOURCE_NAME, BinaryResource.class)) {
