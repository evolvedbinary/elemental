...
public class HTTPUtils {
	 ...
	 */
    public static void addLastModifiedHeader(final Sequence result, final XQueryContext context) {
		if (context.getHttpContext() == null) {
			return;
		}

		@Nullable final ResponseWrapper response = context.getHttpContext().getResponse();
		if (response == null) {
			return;
		}
		...
		try {
			...
			for (final Iterator<DocumentImpl> i = documentSet.getDocumentIterator(); i.hasNext(); ) {
				...
				if (doc != null) {
					...
					if (LOG.isDebugEnabled()) {
						LOG.debug("URI: " + doc.getURI() + " modified: " + doc.getLastModified());
					}
				}
			}
			...
			if (mostRecentDocumentTime > 0) {
				// have to take in account that if the header has already been explicitly set
				// by the XQuery script, we should not modify it .
				if (response.getDateHeader("Last-Modified") == 0) {
					response.setDateHeader("Last-Modified", mostRecentDocumentTime);
				}
			}
