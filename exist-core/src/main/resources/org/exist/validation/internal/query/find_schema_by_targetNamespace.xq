...
declare variable $local:targetNamespace as xs:string+ external;

for $schema in collection($local:collection)//xs:schema[@targetNamespace = $local:targetNamespace]
return
    document-uri(root($schema))