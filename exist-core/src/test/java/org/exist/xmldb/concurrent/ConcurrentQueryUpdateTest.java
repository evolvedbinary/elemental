...
public class ConcurrentQueryUpdateTest extends AbstractConcurrentTest {
	...
	public void setUp() throws Exception {
		...
		try (final EXistResource res = (EXistResource) col.createResource("testappend.xml", "XMLResource")) {
	...
	public void assertAdditional() throws XMLDBException {
		...
		try (final EXistResourceSet result = (EXistResourceSet) service.query("distinct-values(//node/@id)")) {
			assertEquals(41, result.getSize());
			for (int i = 0; i < result.getSize(); i++) {
				try (final EXistResource next = (EXistResource) result.getResource(i)) {
					next.getContent();
				}
			}
		}
