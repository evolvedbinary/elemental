...
public class DeadlockTest {
    ...
    public static class WriterThread extends Thread {
        ...
        public void run() {
            try {
                try (final Collection collection = DatabaseManager.getCollection(XmldbURI.LOCAL_DB, TestUtils.ADMIN_DB_USER, TestUtils.ADMIN_DB_PWD)) {
                    for (int i = 0; i < resources; i++) {
                        try (final EXistResource document = (EXistResource) collection.createResource(Thread.currentThread().getName() + "_" + i, XMLResource.RESOURCE_TYPE)) {
