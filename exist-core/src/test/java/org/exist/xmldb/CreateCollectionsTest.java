...
public class CreateCollectionsTest  {
        ...
        try (final Collection colTest = DatabaseManager.getCollection(XmldbURI.LOCAL_DB + "/" + TEST_COLLECTION)) {
            ...
            try (final Collection testCollection = service.createCollection("test")) {
                ...
                for (final String sampleName : SAMPLES.getShakespeareXmlSampleNames()) {
                    try (final EXistResource res = (EXistResource) storeResourceFromFile(SAMPLES.getShakespeareSample(sampleName), testCollection, sampleName)) {
        ...
        try (final Collection colTest = DatabaseManager.getCollection(XmldbURI.LOCAL_DB + "/" + TEST_COLLECTION)) {
            ...
            try (final Collection testCollection = service.createCollection("test")) {
                ...
                try (final InputStream is = SAMPLES.getMacbethSample();
                     final EXistResource storedFromFile = (EXistResource) storeResourceFromFile(is, testCollection, testFile)) {
                }
                final int resourceCount;
                try (final EXistResource resMacbeth = (EXistResource) testCollection.getResource(testFile)) {
                ...
                try (final EXistResource resMacbethAfterRemove = (EXistResource) testCollection.getResource(testFile)) {
                    assertNull(resMacbethAfterRemove);
                }

                // restore the resource just removed
                try (final InputStream is = SAMPLES.getMacbethSample();
                     final EXistResource restoredFromFile = (EXistResource) storeResourceFromFile(is, testCollection, testFile)) {
                }
                assertEquals("After re-store resource count must increase", resourceCount, testCollection.getResourceCount());
                try (final EXistResource resMacbethAfterRestore = (EXistResource) testCollection.getResource(testFile)) {
    ...
    public void storeBinaryResource() throws XMLDBException, IOException, URISyntaxException {
        try (Collection colTest = DatabaseManager.getCollection(XmldbURI.LOCAL_DB + "/" + TEST_COLLECTION)) {
            ...
            try (Collection testCollection = service.createCollection("test")) {
                ...
                try (final EXistResource storedLogo = (EXistResource) testCollection.getResource("logo.png")) {
    ...
    private XMLResource storeResourceFromFile(final InputStream is, final Collection testCollection, final String fileName) throws XMLDBException, IOException {
        final XMLResource res = (XMLResource) testCollection.createResource(fileName, XMLResource.RESOURCE_TYPE);
    ...
    private byte[] storeBinaryResourceFromFile(Path file, Collection testCollection) throws XMLDBException, IOException {
        try (final EXistResource res = (EXistResource) testCollection.createResource(file.getFileName().toString(), BinaryResource.RESOURCE_TYPE)) {
