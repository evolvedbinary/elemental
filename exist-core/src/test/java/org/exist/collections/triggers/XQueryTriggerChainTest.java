    ...
    static void setup(final BrokerPool pool) throws EXistException, PermissionDeniedException, IOException, SAXException, LockException {
        try (final Txn transaction = pool.getTransactionManager().beginTransaction();
            ...
            try (final Collection collection = broker.getOrCreateCollection(transaction, configCollection1InputUri)) {
                ...
                broker.storeDocument(transaction, configCollection1InputUri.append(DEFAULT_COLLECTION_CONFIG_FILE_URI), new StringInputSource(TRIGGER_1_COLLECTION_CONFIG), xmlMediaType, collection);
            }
    ...
    void xqueryTriggerChain(final BrokerPool pool) throws EXistException, PermissionDeniedException, IOException, SAXException, LockException {
        ...
        try (final Txn transaction = pool.getTransactionManager().beginTransaction();
            ...
            try (final Collection collection = broker.getOrCreateCollection(transaction, COLLECTION_1_INPUT_URI)) {
                assertNotNull(collection);
                final MediaType xmlMediaType = pool.getMediaTypeService().getMediaTypeResolver().fromString(MediaType.APPLICATION_XML);
                broker.storeDocument(transaction, XmldbURI.create(documentName), new StringInputSource(documentContent), xmlMediaType, collection);
            }
