...
public class XQueryTest {
    ...
    public void let() throws XMLDBException, IOException, SAXException {
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.queryResource(NUMBERS_XML, query)) {
            ...
            try (final EXistResource resource = (EXistResource) result.getResource(49)) {
    ...
    public void testFor() throws XMLDBException {
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.queryResource(NUMBERS_XML, query);
             final EXistResource resource = (EXistResource) result.getResource(0)) {
            assertEquals("XQuery: " + query, "3", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("id"));
        }

        query = "for $f in /*/item  order by $f descending  return $f";
        try (final EXistResourceSet result = (EXistResourceSet) service.queryResource(NUMBERS_XML, query);
             final EXistResource resource = (EXistResource) result.getResource(0)) {
            assertEquals("XQuery: " + query, "2", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("id"));
        }

        query = "for $f in /*/item  order by xs:double($f/price) descending  return $f";
        try (final EXistResourceSet result = (EXistResourceSet) service.queryResource(NUMBERS_XML, query);
             final EXistResource resource = (EXistResource) result.getResource(0)) {
            assertEquals("XQuery: " + query, "4", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("id"));
        }
    ...
    public void variable() throws XMLDBException {
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.queryResource(NUMBERS_XML, query)) {
            assertEquals("XQuery: " + query, 2, result.getSize());
            try (final EXistResource resource = (EXistResource) result.getResource(0)) {
                assertEquals("XQuery: " + query, "2", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("name"));
            }
            try (final EXistResource resource = (EXistResource) result.getResource(1)) {
                assertEquals("XQuery: " + query, "3", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("name"));
            }
        }
    }
    ...
    public void typedVariables() throws XMLDBException {
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.query(query)) {
            assertEquals("XQuery: " + query, 2, result.getSize());
            try (final EXistResource resource = (EXistResource) result.getResource(0)) {
                assertEquals("XQuery: " + query, Node.ELEMENT_NODE, ((XMLResource) resource).getContentAsDOM().getNodeType());
                assertEquals("XQuery: " + query, "assign", ((XMLResource) resource).getContentAsDOM().getNodeName());
            }
        }
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.query(query)) {
            ...
            try (final EXistResource resource = (EXistResource) result.getResource(0)) {
                assertEquals("XQuery: " + query, Node.ELEMENT_NODE, ((XMLResource) resource).getContentAsDOM().getNodeType());
                assertEquals("XQuery: " + query, "a", ((XMLResource) resource).getContentAsDOM().getNodeName());
            }
            try (final EXistResource resource = (EXistResource) result.getResource(1)) {
                assertEquals("XQuery: " + query, "1", resource.getContent());
            }
...
//			This one is intercepted by the parser
    ...
    public void namespace() throws XMLDBException, IOException, SAXException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE1_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module1);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE2_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module2);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(NAMESPACED_NAME, XMLResource.RESOURCE_TYPE)) {
    ...
    public void module() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE3_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module3);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE4_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module4);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(FATHER_MODULE_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(fatherModule);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(CHILD1_MODULE_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(child1Module);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(CHILD2_MODULE_NAME, BinaryResource.RESOURCE_TYPE)) {
...
//			Should be a XQST0047 error
    ...
    public void modulesAndNS() throws XMLDBException, IOException, SAXException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE7_NAME, BinaryResource.RESOURCE_TYPE)) {
    ...
    public void importExternalClasspathLibraryModule() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResource doc = (EXistResource) testCollection.createResource("import-external-classpath.xq", BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(
            ...
            service = (EXistXPathQueryService) testCollection.getService("XPathQueryService", "1.0");
        }
    ...
    public void doubleDocNode_2078755() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE8_NAME, BinaryResource.RESOURCE_TYPE)) {
    ...
    public void globalVars() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE5_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module5);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
            }

            try (final EXistResource doc = (EXistResource) testCollection.createResource(MODULE6_NAME, BinaryResource.RESOURCE_TYPE)) {
                doc.setContent(module6);
                doc.setMediaType(MediaType.APPLICATION_XQUERY);
                testCollection.storeResource(doc);
                service = (XQueryService) testCollection.getService("XPathQueryService", "1.0");
            }
        }
    ...
    public void userEscalationForInMemoryNodes() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            final XPathQueryService service = (XPathQueryService) testCollection.getService("XPathQueryService", "1.0");
            try (final EXistResourceSet result = (EXistResourceSet) service.query(query)) {
                try (final EXistResource loggedIn = (EXistResource) result.getResource(0)) {
                ...
                try (final EXistResource currentUser = (EXistResource) result.getResource(1)) {
                ...
                try (final EXistResource currentUserAfterInMemoryOp = (EXistResource) result.getResource(2)) {
    ...
    public void attributeAxis() throws XMLDBException {
        ...
        try (final EXistResourceSet result = (EXistResourceSet) service.query(query);
             final EXistResource resource = (EXistResource) result.getResource(0)) {
            assertEquals("XQuery: " + query, "OK", ((Element) ((XMLResource) resource).getContentAsDOM()).getAttribute("id"));
        }
    }
    ...
    public void retrieveLargeAttribute() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection();
             final EXistResource res = (EXistResource) testCollection.getResource(file_name)) {
            assertNotNull(res);
        }
    }
    ...
    public void evalLoosesContext_1740891() throws XMLDBException {
        ...
        // Store module
        try (final Collection testCollection = getTestCollection()) {
            try (final EXistResource doc = (EXistResource) testCollection.createResource(module_name, BinaryResource.RESOURCE_TYPE)) {
    ...
    public void currentDateTimeInModules_1894009() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResourceSet result = (EXistResourceSet) service.query(query)) {
                assertEquals(2, result.getSize());
                try (final EXistResource resource0 = (EXistResource) result.getResource(0);
                     final EXistResource resource1 = (EXistResource) result.getResource(1)) {
...
//            String output = "<d><b><c>mmm</c></b></d>";
    ...
    public void divYieldsWrongInf_1816496() throws XMLDBException {
        ...
        try (final Collection testCollection = getTestCollection()) {
            ...
            try (final EXistResourceSet result = (EXistResourceSet) service.query(query)) {
                ...
                try (final EXistResource resource = (EXistResource) result.getResource(4)) {
                    assertEquals(query, "-INF", resource.getContent().toString());
                }
                try (final EXistResource resource = (EXistResource) result.getResource(5)) {
                    assertEquals(query, "INF", resource.getContent().toString());
                }

                try (final EXistResource resource = (EXistResource) result.getResource(6)) {
                    assertEquals(query, "NaN", resource.getContent().toString());
                }
                try (final EXistResource resource = (EXistResource) result.getResource(7)) {
                    assertEquals(query, "NaN", resource.getContent().toString());
                }
                try (final EXistResource resource = (EXistResource) result.getResource(8)) {
                    assertEquals(query, "NaN", resource.getContent().toString());
                }
                try (final EXistResource resource = (EXistResource) result.getResource(9)) {
                    assertEquals(query, "NaN", resource.getContent().toString());
                }
                try (final EXistResource resource = (EXistResource) result.getResource(10)) {
                    assertEquals(query, "NaN", resource.getContent().toString());
                }
            }
    ...
    public void wrongAddNamespace_1807014() throws XMLDBException {
        try (final Collection testCollection = getTestCollection()) {
            try (final EXistResource doc = (EXistResource) testCollection.createResource("a.xqy", BinaryResource.RESOURCE_TYPE)) {
    ...
    private XPathQueryService storeXMLStringAndGetQueryService(final String documentName, final String content) throws XMLDBException {
        try (final Collection testCollection = getTestCollection();
             final EXistResource doc = (EXistResource) testCollection.createResource(documentName, XMLResource.RESOURCE_TYPE)) {
            doc.setContent(content);
            testCollection.storeResource(doc);
            return (XPathQueryService) testCollection.getService("XPathQueryService", "1.0");
        }
    }
}
