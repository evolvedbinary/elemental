...
public class GetModuleLoadPathTest {

    private static XmldbURI TEST_COLLECTION_URI = XmldbURI.create("/db/get-module-load-path-test");
    ...
    private static String STANDALONE_MAIN_MODULE_XQ =
            ...
            "document {\n" +
            "  <path>{system:get-module-load-path()}</path>\n" +
            "}";
    ...
    private static String IMPORTING_MAIN_MODULE_XQ =
            ...
            "  <paths>\n" +
            "    <path>{system:get-module-load-path()}</path>\n" +
            "    <library-path>{ilm:module-load-path()}</library-path>\n" +
            "  </paths>\n" +
            "}";
    ...
    public void importingMainModule() throws EXistException, PermissionDeniedException, XPathException, IOException {
        final String expected =
                "<paths>" +
                "<path>.</path>" +
                "<library-path>xmldb:exist://" + IMPORTED_LIBRARY_MODULE_URI.removeLastSegment().getCollectionPath() + "</library-path>" +
                "</paths>";
    ...
    public void storedImportingMainModule() throws EXistException, PermissionDeniedException, XPathException, IOException {
        final String expected =
                ...
                "<library-path>" + IMPORTED_LIBRARY_MODULE_URI.removeLastSegment().getCollectionPath() + "</library-path>" +
                "</paths>";
