...
public class TwoDatabasesTest {
    ...
    @RegisterExtension
    public final EmbeddedDatabaseExtension embeddedDatabase1 = new EmbeddedDatabaseExtension("db1", config1File, null, true);

    @RegisterExtension
    public final EmbeddedDatabaseExtension embeddedDatabase2 = new EmbeddedDatabaseExtension("db2", config2File, null, true);

    private Subject user1;
    private Subject user2;

    @BeforeEach
    void setUp() throws EXistException, PermissionDeniedException {
        final BrokerPool pool1 = embeddedDatabase1.getBrokerPool();
        ...
        try(final DBBroker broker1 = pool1.get(Optional.of(user1))) {
            ...
            assertNotSame(top1, null);
        }

        final BrokerPool pool2 = embeddedDatabase2.getBrokerPool();
    ...
    private Collection storeBin(final DBBroker broker, final Txn txn, String suffix) throws PermissionDeniedException, LockException, SAXException, EXistException, IOException {
        ...
        broker.storeDocument(txn, XmldbURI.create("bin"), new StringInputSource(data.getBytes(UTF_8)), txtMediaType, top);
        return top;
    }
