    ...
    public void restoreConflictingUsername() throws PermissionDeniedException, EXistException, SAXException, ParserConfigurationException, IOException, URISyntaxException, XMLDBException, IllegalAccessException, ClassNotFoundException, InstantiationException {
        ...
        try (final Collection test = cms.createCollection("test")) {
            ...
            try (final Collection frankTest = DatabaseManager.getCollection("xmldb:exist:///db/test", FRANK_USER, FRANK_USER)) {
                try (final EXistResource frankDoc = (EXistResource) frankTest.createResource(FRANKS_DOCUMENT, "XMLResource")) {
            ...
            try (final Collection jackTest = DatabaseManager.getCollection("xmldb:exist:///db/test", JACK_USER, JACK_USER)) {
                try (final EXistResource jackDoc = (EXistResource) jackTest.createResource(JACKS_DOCUMENT, "XMLResource")) {
            ...
            try (final EXistResource fDoc = (EXistResource) test.getResource(FRANKS_DOCUMENT)) {
            ...
            try (final EXistResource jDoc = (EXistResource) test.getResource(JACKS_DOCUMENT)) {
